...
def main(data_dir='data/RPI/', file_name='R_outside_rotated'):
    #select dataset
    
    #select TCP
    with open('config/tormach_za06_robot_default_config.yml') as robot_file:
        with open('config/default_tcp.yaml') as tool_file:
            robot_ATI=yml2robdef(robot_file,tool_file)

    with open('config/workspace_H.yaml') as f:
        H_base_curve = np.array(yaml.safe_load(f),dtype=np.float64)
    
    ###read in curves
    curve = read_csv(data_dir+file_name+'.csv',header=None).values
    ...
    curve_base,curve_normal_base=curve_frame_conversion(curve[:,:3],curve[:,3:],H_base_curve)
    ...
    curve_js=find_js_qp(robot_ATI,curve_base,curve_normal_base)
    ...
    df.to_csv(data_dir+file_name+'_in_base_frame.csv',header=False,index=False)
    DataFrame(curve_js).to_csv(data_dir+file_name+'_js.csv',header=False,index=False)
    with open(data_dir+'curve_pose.yaml', 'w') as file:
        documents = yaml.dump({'H':H_base_curve.tolist()}, file)
