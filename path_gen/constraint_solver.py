...
class lambda_opt(object):
		...
		for i in range(len(curve)):
			...
			try:
				...
				while error_fb>0.01:
					...
					qdot=solve_qp(H,f,solver='quadprog',lb=self.robot1.lower_limit-q_all[-1]+self.lim_factor*np.ones(6),ub=self.robot1.upper_limit-q_all[-1]-self.lim_factor*np.ones(6))
	...
	def single_arm_stepwise_optimize2(self,q_init,lamdot_des,curve=[],curve_normal=[]):
		...
		while lam_qp<lam[-1] and idx<len(curve)-1:
			...
			qdot=solve_qp(H,f,solver = 'quadprog',lb=lb,ub=ub)
			qdot_prev=qdot
		...
		for i in range(len(self.curve)):
			try:
				...
				while error_fb>0.1:
					...
					qdot=solve_qp(H,f,solver = 'quadprog',lb=lower_limit-np.hstack((q_all1[-1],q_all2[-1]))+self.lim_factor*np.ones(12),ub=upper_limit-np.hstack((q_all1[-1],q_all2[-1]))-self.lim_factor*np.ones(12))
		...
		for i in range(len(self.curve)):
			...
			try:
				...
				while error_fb>0.2:
					...
					q1dot=solve_qp(H,f,solver='quadprog',
                                   A=J1p,b=np.zeros(3),lb=self.robot1.lower_limit-q_all1[-1]+self.lim_factor*np.ones(6),ub=self.robot1.upper_limit-q_all1[-1]-self.lim_factor*np.ones(6))
					...
					q2dot=solve_qp(H,f,solver='quadprog',
                                   A=J2R,b=np.zeros(3),lb=self.robot2.lower_limit-q_all2[-1]+self.lim_factor*np.ones(6),ub=self.robot2.upper_limit-q_all2[-1]-self.lim_factor*np.ones(6))
